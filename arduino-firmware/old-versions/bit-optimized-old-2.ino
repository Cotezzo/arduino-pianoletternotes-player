...
const uint8_t PROGMEM notes_layer_SX[] = {
...
const uint8_t PROGMEM notes_layer_DX[] = {
...
};  // Generate 0x values instead (fa figo)
...
void playMelody() {
  ...
  // SX variables init
  int raw_indexes_SX = -1;
  byte mask_SX_1 = 0b11111111;
  byte mask_SX_2 = 0b11111111;
  int indexCorrection_SX = 0;
  byte shiftCorrection_SX = 7;

  int raw_indexes_DX = -1;
  byte mask_DX_1 = 0b11111111;
  byte mask_DX_2 = 0b11111111;
  int indexCorrection_DX = 0;
  byte shiftCorrection_DX = 7;
  ...
  while (i < songSize) {
    ...
    // Common
    const byte mapIndex = i / 8;
    const byte mapBitIndex = 7 - (i % 8);
    const byte isNote_SX = bitRead(map_layer_SX[mapIndex], mapBitIndex); // Example: i = 5, number 0, 6th bit.
    const byte isNote_DX = bitRead(map_layer_DX[mapIndex], mapBitIndex); // Example: i = 5, number 0, 6th bit.
    ...
    if (!isNote_SX && !isNote_DX)
      ...
      if (isNote_SX) {
        ...
        if (mask_SX_2 == 0b00000000) {  // Reset variables every 8 cycles and apply correction
          mask_SX_2 = ~mask_SX_2;       // Indirectly set mask_1 to 0b00000000, mask_2 to 0b11111110
          raw_indexes_SX--;             // This and delete completely indexCorrection
          shiftCorrection_SX = 7;
        }
        ...
        buffer[0] = pgm_read_byte(&notes_layer_SX[raw_indexes_SX - indexCorrection_SX]);      // note chunk #1
        buffer[1] = pgm_read_byte(&notes_layer_SX[raw_indexes_SX + 1 - indexCorrection_SX]);  // note chunk #

        // Use bit masks and shifts to get the 7-bits number from the PROGMEM notes layer
        const byte note_index_SX = ((buffer[0] & mask_SX_1) << shiftCorrection_SX ) | ((buffer[1] & mask_SX_2) >> (8 - shiftCorrection_SX));
        shiftCorrection_SX--;   // Decrement shift correction (will reset to 7 when 0 aka when mask_2 == 0b00000000)
        raw_indexes_SX++;   // Next index MAYBE DECREMENT THIS INSTEAD OF INDEXCORRECTION
      ...
      if (isNote_DX) {
        ...
        if (mask_DX_2 == 0b00000000) {  // Reset variables every 8 cycles and apply correction
          mask_DX_2 = ~mask_DX_2;       // Indirectly set mask_1 to 0b00000000, mask_2 to 0b11111110
          raw_indexes_DX--;             // This and delete completely indexCorrection
          shiftCorrection_DX = 7;
        }
        ...
        byte buffer[2];     // Save necessary values from PROGMEM into buffer
        buffer[0] = pgm_read_byte(&notes_layer_DX[raw_indexes_DX - indexCorrection_DX]);      // note chunk #1
        buffer[1] = pgm_read_byte(&notes_layer_DX[raw_indexes_DX + 1 - indexCorrection_DX]);  // note chunk #

        // Use bit masks and shifts to get the 7-bits number from the PROGMEM notes layer
        const byte note_index_DX = ((buffer[0] & mask_DX_1) << shiftCorrection_DX ) | ((buffer[1] & mask_DX_2) >> (8 - shiftCorrection_DX));
        shiftCorrection_DX--;   // Decrement shift correction (will reset to 7 when 0 aka when mask_2 == 0b00000000)
        raw_indexes_DX++;   // Next index MAYBE DECREMENT THIS INSTEAD OF INDEXCORRECTION
